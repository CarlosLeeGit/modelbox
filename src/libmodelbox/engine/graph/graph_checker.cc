...
std::string LeastCommonAncestor::GetMatchPortName(int match_a, int match_b,
  ...
  std::shared_ptr<Node> node_a =
      CastNode(all_nodes_[index_name_map_[match_a]]);
  std::shared_ptr<Node> node_b =
      CastNode(all_nodes_[index_name_map_[match_b]]);
  std::shared_ptr<Node> match =
      CastNode(all_nodes_[index_name_map_[match_node]]);
...
std::pair<std::string, std::string> LeastCommonAncestor::Find(
  ...
  if (node_a == node_b) {
    return std::pair<std::string, std::string>(node_a, "");
  }
  ...
  for (auto port : outports) {
    ...
    for (auto linkport : linkPorts) {
      connect_nodes.insert(
          CastNode(linkport->GetNode()));
    }
  }
}

void OverHierarchyCheck::SetColorType(std::shared_ptr<Node> node, int color) {
  if (node->GetConditionType() == IF_ELSE) {
    color_type_[color] = Color_Type::CONDITION;
    return;
  }

  if (node->GetOutputType() == EXPAND) {
    color_type_[color] = Color_Type::EXPAND_COLLAPSE;
    return;
  }

  if (node->GetLoopType() == LOOP) {
    color_type_[color] = Color_Type::CIRCLE;
    return;
  }
}
...
void OverHierarchyCheck::GetNodeType(
  ...
  for (auto &input_port : input_ports) {
    ...
    auto pre_real_match_node = CastNode(pre_match_node);
  ...
  for (auto &start_node : start_nodes_) {
    ...
    while (!stack.empty()) {
      ...
      for (auto &connect_node : connect_nodes) {
        ...
        status = GetNewColor(color, level, connect_node);
        if (status != STATUS_OK) {
        ...
        if ((connect_type == OHC_Type::END || connect_type == OHC_Type::MID) &&
            color_type_[color.back()] == Color_Type::CONDITION) {
          continue;
        }
...
void GraphChecker::SetMatchNodes() {
  for (auto &node : all_nodes_) {
    ...
    for (auto &single_node_match : graph_single_port_match_map_[node.first]) {
      auto match_condition_node = CastNode(
          all_nodes_.at(single_node_match.second));
...
Status GraphChecker::CheckBranchPathMatch(const std::string &start,
  ...
  if (end == start) {
    ...
    if (end_node->GetOutputType() == EXPAND) {
      expands_++;
    }
...
bool GraphChecker::CheckPortMatch(
  ...
  std::shared_ptr<Node> node =
      CastNode(all_nodes_[match_pair.first]);
...
Status GraphChecker::CheckNodeMatch(
  ...
  for (auto iter = node_stream_map.begin(); iter != node_stream_map.end();
    ...
    auto single_match_real_node =
        CastNode(all_nodes_[single_match_node.first]);
    ...
    single_port_match_map_[iter->first] = single_match_node.first;
  ...
  if (single_match_result.size() == 1) {
    graph_match_map_[node_name] = single_match_result[0].first;
    return status;
  }
  ...
  auto multi_match_real_node =
      CastNode(all_nodes_[multi_match_node.first]);
...
std::pair<std::string, std::string> GraphChecker::LeastCommonAncestors(
  ...
  for (size_t i = 1; i < match_nodes.size(); ++i) {
    ...
    if (!res_node.second.empty()) {
      continue;
    }

    if (tmp_node.second == match_nodes[i].second) {
      res_node.second = tmp_node.second;
    } else {
      auto input_ports = all_nodes_[res_node.first]->GetInputPorts();
      if (input_ports.size() == 0) {
        res_node.second = "external";
      } else {
        res_node.second = input_ports[0]->GetName();
      }
    }
  }
